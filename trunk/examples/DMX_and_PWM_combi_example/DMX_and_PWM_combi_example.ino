...
Cct * cctLamp  =  new Cct( 5, 6, true ); // Use pin 5 for warm white, and pin 6 for cool white
RGB * rgbLamp  =  new RGB( 1, false );   // Use channel 1 for red; chennel 2 and 3 are inferred from that, but can also be set manually
...
void setup()
{
  ...
  cctLamp->intensityTo( 255, 1000 ); // Make sure the light is turned on so we can see what is happening
}
...
void loop()
{
  ...
  **/
  ...
  if( !cctLamp->isAnimating()) // Returns true if the LED is in an animation
  {
    if (cctLamp->getCct() == 0) // Returns the current CCT Value of the LED
    {
      cctLamp->cctTo( 100, 2000 ); // Sets the desired CCT value and the time (in millis) it should take to get there
    }
    else {
      cctLamp->cctTo( 0, 2000 );
    }
  }
